...
| S-4 | Calibration **routing selects on the same gold set it is scored on** (in-sample, no held-out split). | **FIXED** — added `calibration_routed_holdout`: leave-one-out routing (select argmin-Brier on the train fold, predict the held-out point) = an unbiased generalisation estimate. acc 1.0 but **Wilson CI [0.806, 1.0]** at n=16: a point estimate is not a generalisation claim (Sutskever). |
| S-6 | A naive **percentile bootstrap degenerates to [1,1]** on an all-success vector, overstating certainty. | **FIXED/DISCLOSED** — held-out CI uses **Wilson** (correct at the boundary); the degenerate bootstrap is shown alongside and labelled, with a pure test pinning the pitfall. |
